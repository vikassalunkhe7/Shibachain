...
class Slide {
  constructor(el) {
    ...
    this.DOM.titleInner = this.DOM.title.querySelector('.slide__title-link')
    ...
    this.DOM.button = this.DOM.el.querySelectorAll('.slide__nav-button')
    this.DOM.numbers = this.DOM.el.querySelectorAll('.slide__numbers')
    // Some config values.
    this.config = {
      revealer: {
    ...
    return new Promise((resolve, reject) => {
      if (opts.animation) {
        ...
        this.DOM.revealer.style.transform =
          action === 'hide' ? 'translate3d(0,0,0)' : `translate3d(0,${opts.direction === 'prev' ? '100%' : '-100%'},0)`
  ...
  toggle(action, direction, delay) {
    ...
    return this[action === 'hide' ? 'showRevealer' : 'hideRevealer']({ delay, direction, animation: true })
  }
  ...
  toggleTexts(action, direction, delay) {
    if (this.isPositionedCenter()) {
      ...
      TweenMax.to(this.DOM.explore, speed, {
        ease,
        delay: delay + 0.2,
        startAt: action === 'show' ? { y: '30px' } : {},
        y: action === 'hide' ? '-30px' : '0%',
        opacity: action === 'hide' ? 0 : 1
      })

      TweenMax.to(this.DOM.numbers, speed, {
        ease,
        delay: delay + .4,
        opacity: action === 'hide' ? 0 : 1
      })
...
class Slideshow {
  ...
  render(hideSlidesBefore = false) {
    ...
    this.prevSlide = this.slides[this.current + 2 <= this.slidesTotal - 1 ? this.current + 2 : 0]

    if ( this.current === this.slidesTotal - 1 ) {
      this.nextSlide = this.slides[0]
      this.prevSlide = this.slides[1]
    }
  ...
  initEvents() {
    ...
    for (const slide of this.slides) {
      ...
      Array.prototype.forEach.call(slide.DOM.button, (button, i) => {

        button.addEventListener('click', event => {
          if (button.classList.contains('slide__nav-button--next')) {
            this.navigateWithButton('next')
          } else if (button.classList.contains('slide__nav-button--prev')) {
            this.navigateWithButton('prev')
          }
  ...
  toggleSlides(action, direction) {
    ...
    ;[this.nextSlide, this.currentSlide, this.prevSlide].forEach(slide => {
      const delay = slide.isPositionedCenter()
        ? delayFactor / 2
        : direction === 'next'
  ...
  navigate(direction) {
    ...
    if (direction === 'next') {
      ...
      this.current = this.current < this.slidesTotal - 1 ? this.current + 2 : 0;
    }
